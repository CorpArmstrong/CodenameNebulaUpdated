//=============================================================================
// AiUWindowListBox - a listbox
//=============================================================================
class AiUWindowListBox extends AiUWindowListControl;

var float				ItemHeight;
var AiUWindowVScrollbar	VertSB;
var AiUWindowListBoxItem	SelectedItem;
...
var bool				bDragging;
var float				DragY;
var AiUWindowListBox		DoubleClickList;	// list to send items to on double-click
...
{
	local AiUWindowListBoxItem OverItem;
...
function AiUWindowListBoxItem GetItemAt(float MouseX, float MouseY)
{
	local float y;
	local AiUWindowList CurItem;
	local int i;
	...
	for(y=0;(y < WinHeight) && (CurItem != None);CurItem = CurItem.Next)
	{
		if(CurItem.ShowThisItem())
		{
			if(MouseY >= y && MouseY <= y+ItemHeight)
				return AiUWindowListBoxItem(CurItem);
			y = y + ItemHeight;
		}
	}
...
function SetSelectedItem(AiUWindowListBoxItem NewSelected)
{
...
{
	local AiUWindowListBoxItem NewSelected;
...
function ReceiveDoubleClickItem(AiUWindowListBox L, AiUWindowListBoxItem I)
{
	I.Remove();
	Items.AppendItem(I);
	SetSelectedItem(I);
...
function DoubleClickItem(AiUWindowListBoxItem I)
{
...
function bool ExternalDragOver(AiUWindowDialogControl ExternalControl, float X, float Y)
{
	local AiUWindowListBox B;
	local AiUWindowListBoxItem OverItem;

	// Subclass should return false and not call this version if this external
	// drag should be denied.

	B = AiUWindowListBox(ExternalControl);
