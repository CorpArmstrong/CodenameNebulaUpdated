class AiUWindowTabControlTabArea extends AiUWindowWindow;

var int TabOffset;
var bool bShowSelected;
var AiUWindowTabControlItem FirstShown;
var bool bDragging;
var AiUWindowTabControlItem DragTab;
var int TabRows;
...
{
	local AiUWindowTabControlItem I, Selected, LastHidden;
	...
	TabCount=0;
	for(
			I = AiUWindowTabControlItem(AiUWindowTabControl(ParentWindow).Items.Next);
			I != None;
			I = AiUWindowTabControlItem(I.Next)
		)
	{
	...
	Selected = AiUWindowTabControl(ParentWindow).SelectedTab;
	...
	AiUWindowTabControl(ParentWindow).LeftButton.bDisabled = TabOffset <= 0;
	AiUWindowTabControl(ParentWindow).RightButton.bDisabled = !bHaveMore;
	TabRows = 1;
}

function SizeTabsMultiLine(Canvas C)
{
	local AiUWindowTabControlItem I, Selected;
	local float W, H;
	local int MinRow;
...
{
	if(AiUWindowTabControl(ParentWindow).bMultiLine)
		SizeTabsMultiLine(C);
	else
...
function LMouseDown(float X, float Y)
{
		...
		)
	{
		...
		if( X >= I.TabLeft && X <= I.TabLeft + I.TabWidth && (TabRows==1 || (Y >= I.TabTop && Y <= I.TabTop + I.TabHeight)) )
		{
			...
			AiUWindowTabControl(ParentWindow).GotoTab(I, True);
		}
	}
}
...
function DrawItem(Canvas C, AiUWindowList Item, float X, float Y, float W, float H, bool bShowText)
{
	if(Item == AiUWindowTabControl(ParentWindow).SelectedTab)
		LookAndFeel.Tab_DrawTab(Self, C, True, FirstShown==Item, X, Y, W, H, AiUWindowTabControlItem(Item).Caption, bShowText);
	else
		LookAndFeel.Tab_DrawTab(Self, C, False, FirstShown==Item, X, Y, W, H, AiUWindowTabControlItem(Item).Caption, bShowText);
}
