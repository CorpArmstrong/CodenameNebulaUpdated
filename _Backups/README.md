...
| 5 | `Phase5_Orphans/UTX/` | **1 orphan texture package:** AIStalk.utx (4.2 MB). Zero references in UC code OR any `.dx` map (cross-checked). PFADTex.utx was initially quarantined here too but **restored to `Textures/` in commit `641648b`** — 3 maps reference it via the `'PFAD'` asset prefix, which the Phase 5 grep missed (it searched for the literal package name). The other 5 packages initially flagged as orphans by the UC-only scan are actually used by maps — see `Phase5_6_Audit.md`. |
