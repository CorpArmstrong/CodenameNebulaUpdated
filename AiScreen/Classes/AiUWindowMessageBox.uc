class AiUWindowMessageBox expands AiUWindowFramedWindow;
...
{
	WindowTitle = Title;
	AiUWindowMessageBoxCW(ClientArea).SetupMessageBoxClient(Message, Buttons, InEnterResult);
	Result = InESCResult;
	TimeOutTime = 0;
	TimeOut = InTimeOut;
	FrameCount = 0;
...
function BeforePaint(Canvas C, float X, float Y)
{
	...
	if(!bSetupSize)
	{
		...
		SetSize(200, (WinHeight - R.H) + AiUWindowMessageBoxCW(ClientArea).GetHeight(C));
...
defaultproperties
{
     ClientClass=class'AiUWindow.UWindowMessageBoxCW'
}
