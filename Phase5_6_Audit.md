...
The initial audit (generated by an Explore subagent) had **three systematic blind spots**. Each was caught only when the compile broke after moving "orphan" classes. The corrected picture below supersedes the original analysis further down in this document.

### Blind spots

1. **`Class'CNN.X'` pattern in `defaultproperties` tuples** — the agent matched `extends`/`spawn(class'X')` but missed class references stored as values inside assignment tuples like `buttonDefaults(0)=(Invoke=Class'CNN.ApocalypseInsideMenuSelectDifficulty')`. Result: the entire 5-class Apocalypse Menu was misclassified as orphan; it is actually the production New Game menu flow invoked by `CNNMenuMainTest.StartNewGame()` at [CNNMenuMainTest.uc:12](CNN/Classes/CNNMenuMainTest.uc#L12).

2. **`.con` conversation file references** — conversations store speaker/pawn class names as binary strings. The agent never scanned `.con` files. `BodyBox` has 148 hits across 4 `.con` files (OpheliaDocksAndL1 + OpheliaL2ArmMagdalene, mirrored in `CNN/Conversations/` and `CNNText/Conversations/`), `CNNSecurityCameraAlarmTrigger` has 4 hits. Both were misclassified as orphan.

3. **`.dx` map name-table references for UC classes** — the agent cross-checked textures against `.dx` strings (at the user's insistence) but didn't do the same for UC class names. Direct `grep -a` against shipped maps revealed:
   - `AiGlassFragment` → 2 maps (05_MoonIntro, 06_OpheliaL1)
   - `CnnDispatcher32` → 1 map (06_OpheliaL1)
   - `GlobeBall` → 5 maps (Hijacking, Mutiny, OpheliaL2, OpheliaL2_QuestSystem, Transcend)
   - `ZeroClient` → 6 maps

### True orphan count after corrections

| Category | Agent said | Actual |
|---|---|---|
| UC classes | 13 orphans | **1 orphan** (BarrelBoomImportScript only) |
| Texture packages | 7 orphans | 2 orphans (AIStalk, PFADTex) |
| Prefab files | 19 orphans | 17 orphans (04_NYC_UNATCOHQ.t3d/.mtl + 15 prefabs) |
| Utility .exe | 2 orphans | 2 orphans (MPMaps.exe, TextureFiles.exe) |

### Methodology lesson

For the next pass on any mod of this type, reachability analysis MUST cross-reference class names against, at minimum:
- All `.uc` files (including `defaultproperties` tuple values)
- All `.dx` map name tables (use `grep -a`)
- All `.con` conversation files
- All config files (`.ini`)

Textures and sound packages follow the same rule.

---

## Summary Counts (ORIGINAL — partially corrected above)
