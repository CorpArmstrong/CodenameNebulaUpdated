...
//=============================================================================
class AiWindowConsole extends Console;

var AiUWindowRootWindow	Root;
...
var class<AiUWindowConsoleWindow> ConsoleClass;
...
var AiUWindowConsoleWindow ConsoleWindow;
...
function CreateRootWindow(Canvas Canvas)
{
	...
	Root = New(None) class<AiUWindowRootWindow>(DynamicLoadObject(RootWindow, class'Class'));
	...
	ConsoleWindow = AiUWindowConsoleWindow(Root.CreateWindow(ConsoleClass, 100, 100, 200, 200));
	if(!bShowConsole)
		HideConsole();

	AiUWindowConsoleClientWindow(ConsoleWindow.ClientArea).TextArea.AddText(" ");
	for (I=0; I<4; I++)
		AiUWindowConsoleClientWindow(ConsoleWindow.ClientArea).TextArea.AddText(MsgText[I]);
}

function RenderUWindow( canvas Canvas )
{
	local AiUWindowWindow NewFocusWindow;
...
event Message( PlayerReplicationInfo PRI, coerce string Msg, name N )
{
	...
	if( Msg!="" )
	{
		...
		if (ConsoleWindow != None)
			AiUWindowConsoleClientWindow(ConsoleWindow.ClientArea).TextArea.AddText(OutText);
	}
}
...
event AddString( coerce string Msg )
{
	...
	{
		if (ConsoleWindow != None)
			AiUWindowConsoleClientWindow(ConsoleWindow.ClientArea).TextArea.AddText(Msg);
	}
}
