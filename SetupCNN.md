...
- **Legacy renderers crash on Windows 10/11** — `D3DDrv.D3DRenderDevice` (the 1999 D3D7 device) fails with `Failed resetting mode` / `HandleBigChange <- UD3DRenderDevice::Lock`, and `GlideDrv` isn't shipped at all. A never-configured retail/CD install inherits exactly these from the game's `Default.ini`, together with 640x480 @ **16-bit**, and Windows no longer exposes 16-bit fullscreen modes. Both INI generators (`tools/generate_cnn_ini.ps1` and `CNNInstallUtil`) now rewrite such renderers to `D3D9Drv.D3D9RenderDevice` and force 32-bit colour, so the mod can't inherit an unbootable config. If D3D9 misbehaves on a given exe, fall back to `OpenGlDrv.OpenGLRenderDevice` (ships with every 1112fm install).
- **The in-game renderer picker can undo that** — Settings > "Rendering Device..." runs `RELAUNCH -changevideo`, whose dialog is built from the `.int` files in the game `System/`. `D3DDrv.int` advertises "Direct3D support" next to "Direct3D9 support", so it's easy to select the dead 1999 device and get a black screen. Base-game `.int` files are left untouched by design; run `tools/fix_renderer.ps1` to put the INI back on a working renderer.
- **Monitor black while the game renders fine** — if a screen-share shows the menu but the display is black, only fullscreen presentation is failing. Run `tools/fix_renderer.ps1 -Windowed` to force `StartupFullscreen=False`, then rule out HDR, a second monitor, and refresh-rate mismatch.
